...
DATASET_URL_1 = "https://prod-dcd-datasets-cache-zipfiles.s3.eu-west-1.amazonaws.com/zr7vgbcyr2-1.zip"
DATASET_FILE_1 = "zr7vgbcyr2-1.zip"
DATASET_SUM_1 = "1ac1f5078624fa08280307901e4e916f"
...
def extract_dataset(dataset_root_path: str):
    ...
    image_files = [
        "original_files/images/imgs_part_1.zip",
        "original_files/images/imgs_part_2.zip",
        "original_files/images/imgs_part_3.zip",
    ]
    for zip_file in image_files:
        run_cmd_get_output(["unzip", "-qq", dataset_root_path + zip_file])
...
def parse_dataset(
    ...
    # create a class dict
    df = pd.read_csv(dataset_root_path + "original_files/metadata.csv")
    class_list = df["diagnostic"].unique()
    for k, class_name in enumerate(sorted(class_list, key=str.casefold)):
        class_dict[class_name] = k

    for image_path in image_path_list:
        problem_value, set_value, subject_id_from_path = 0, 0, -1
        ...
        if problem_value != 0:
            ...
            class_from_path = df[df["img_id"] == patient_id]["diagnostic"].iloc[0]
            subject_id_from_path = int(
                df[df["img_id"] == patient_id]["patient_id"].iloc[0].split("_")[-1]
            )
        ...
        image_list.append(
            DatasetImage(
                ...
                subject_id=subject_id_from_path,
            )
        )
