...
# 1) under the directory original_files, there is a duplicate directory oct2017 which contains another
# two duplicates of the whole dataset
# 2) the OCT2017\ /folder name has a space at the end!
...
DATASET_URL_1 = "https://www.kaggle.com/datasets/paultimothymooney/kermany2018/download?datasetVersionNumber=2"
DATASET_FILE_1 = "archive.zip"
DATASET_SUM_1 = "ac082d1f8e94840414b540a77def81e4"
...
def parse_dataset(
    ...
    class_list = os.listdir(
        dataset_root_path + "original_files/" + "OCT2017 /" + "train/"
    )
    ...
    for image_path in image_path_list:
        ...
        if "oct2017/" in image_path:
            problem_value = 3

        # set split to test if image is in the test set folder
        if "OCT2017 /val" in image_path:
            set_value = 1
        if "OCT2017 /test" in image_path:
            set_value = 2
        ...
        try:
            subject_id = int(image_path.split("/")[-1].split("-")[1])
        except:
