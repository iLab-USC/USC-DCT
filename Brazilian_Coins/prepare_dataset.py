...
#   + Kaggle Dataset
#   + Dataset duplication + regression version of the dataset
...
DATASET_URL_1 = (
    "https://www.kaggle.com/datasets/lgmoneda/br-coins/download?datasetVersionNumber=2"
)
DATASET_FILE_1 = "archive.zip"
DATASET_SUM_1 = "364242cf4ef4fd89e411311c3cb6e724"

ONLY_VALID_DATASET_FOLDER = "original_files/classification_dataset/all/"
DUPLICATE_FILE_LIST = [
    # Duplicate Dataset with Different Labeling Structure
    "COCO_labelme_classification/classification/",
    "COCO_labelme_classification/classification/coco/",
    "COCO_labelme_classification/classification/coco/JPEGImages/",
]
NONDATASET_FILE_LIST = [
    # Similar Dataset using "Counting" Instead
    "COCO_labelme_regression/",
    "COCO_labelme_regression/regression/",
    "COCO_labelme_regression/regression/coco/",
    "COCO_labelme_regression/regression/coco/JPEGImages/",
    "regression_dataset/all/",
...
def parse_dataset(
    ...
    # Using prescribed class list
    class_list = [5, 10, 25, 50, 100]
    for id, classNum in enumerate(class_list):
        class_dict.update({str(classNum): id})
    ...
    for image_path in sorted(image_path_list):
        ...
        # If Image is in a non-dataset folder
        if ONLY_VALID_DATASET_FOLDER not in image_path:
            if DUPLICATE_FILE_LIST[0] not in image_path:
                problem_value = 2
            else:
                problem_value = 3
        elif not is_image_valid(dataset_root_path + image_path):
            problem_value = 1

        else:
            class_name_from_path = image_path.split("/all/")[-1].split("_")[0]
            assert class_name_from_path in class_dict
            class_id = class_dict[class_name_from_path]

        # regression is a separate task
        if "regression" in image_path.lower():
            problem_value = 6
