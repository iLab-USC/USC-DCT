...
#
# 1) archive has dataset-master, which is unprocessed raw dataset with segmentation labels, removed from classification task
# 2) labels.csv seems to be only for segmentation dataset, doesn't make sense for the classification dataset
...
DATASET_URL_1 = "https://www.kaggle.com/datasets/paultimothymooney/blood-cells"
DATASET_FILE_1 = "archive.zip"
DATASET_SUM_1 = "004d14203442d22156e5048d1cdcc94d"
...
def parse_dataset(
    ...
    path_to_dataset_splits = (
        dataset_root_path
        + "original_files/"
        + "dataset2-master/dataset2-master/"
        + "images/"
    )
    directories_train = os.listdir(path_to_dataset_splits + "TRAIN")
    directories_test = os.listdir(path_to_dataset_splits + "TEST")
    directories_test2 = os.listdir(path_to_dataset_splits + "TEST_SIMPLE")

    assert set(directories_train) == set(directories_test)
    assert set(directories_train) == set(directories_test2)
    assert set(directories_test2) == set(directories_test)

    class_list = [d for d in directories_train]
    ...
    for image_path in image_path_list:
        ...
        if "dataset-master/dataset-master" in image_path:
            problem_value = 4

        # set split for test folders
        if "images/TEST" in image_path:
            set_value = 2
