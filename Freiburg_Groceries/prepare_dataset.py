...
#
# - Taking dataset defined class ids from github repo
# - 5 different train/test splits given in repo, marking all images train

#
# 1. constants
#

# 2 files are necessary for this example dataset
...
DATASET_URL_2 = (
    "https://github.com/PhilJd/freiburg_groceries_dataset/archive/refs/heads/master.zip"
)
DATASET_FILE_2 = "master.zip"
DATASET_SUM_2 = "598876dd8ec6b9d4476fe0de4eac890e"

NONDATASET_FILE_LIST = [
    "freiburg_groceries_dataset-master/figures/class_overview.png",
    "freiburg_groceries_dataset-master/figures/examples.png",
]
...
def extract_dataset(dataset_root_path: str):
    ...
    original_file_path = dataset_root_path + "archives/" + DATASET_FILE_2
...
def parse_dataset(
    ...
    print("parse dataset")
    # list of images (file, class_id, set, problem)
    ...
    directories = os.listdir(dataset_root_path + "original_files/" + "images")

    # get class names from provided meta/classes.txt
    class_list_file = (
        dataset_root_path
        + "original_files/"
        + "freiburg_groceries_dataset-master/classid.txt"
    )
    with open(class_list_file, "r") as f:
        rows = f.readlines()
        class_list = [(row.split()[0].strip(), row.split()[1].strip()) for row in rows]

    # run sanity check that class names match
    assert set(directories) == set([c[0] for c in class_list])

    # update class_dict with class indices
    for class_info in class_list:
        class_dict.update({class_info[0]: int(class_info[1])})

    # go through each image, match to flag problems,
    for image_path in image_path_list:
        problem_value, set_value = 0, 0

        for f in NONDATASET_FILE_LIST:
            if f in image_path:
                problem_value = 2
