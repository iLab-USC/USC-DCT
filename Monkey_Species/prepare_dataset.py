...
# 1) there is train and validation split but no test
# 2) class_names are provided in the monkey_labels.txt file
# 3) uses the common_name as the class_name from the monkey_labels.txt
...
DATASET_URL_1 = "https://www.kaggle.com/datasets/slothkong/10-monkey-species/download?datasetVersionNumber=2"
DATASET_FILE_1 = "archive.zip"
DATASET_SUM_1 = "135bcadbee1904940336691cef8f89ff"
...
def parse_dataset(
    ...
    class_list = os.listdir(dataset_root_path + "original_files/" + "training/")

    # create the class_name to label mappings
    label_df = pd.read_csv(dataset_root_path + "original_files/" + "monkey_labels.txt")
    # create the img_id to class_id mapping
    label_dict = label_df.set_index("Label").to_dict(orient="index")
    label_dict = {
        k.strip(" "): v[" Common Name                   "].strip(" ")
        for k, v in label_dict.items()
    }

    # # update class_dict with class indices
    for k, class_name in enumerate(sorted(label_dict)):
        class_dict.update({label_dict[class_name]: k})

    mapped_class_dict = dict()
    # update label dict
    for k, class_name in enumerate(sorted(label_dict)):
        mapped_class_dict.update({class_name: k})
    ...
    for image_path in image_path_list:
        ...
        # set split to test if image is in the test set folder
        ...
        if problem_value != 0:
            ...
            assert class_from_path in mapped_class_dict
            class_int = mapped_class_dict[class_from_path]
