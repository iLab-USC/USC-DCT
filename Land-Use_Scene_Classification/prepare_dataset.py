...
# 1) has train/val/test split
# 2) provides a class list, with ids
# 3) Although it appears like the 'images' folder is entirely full of duplicates,
#    in actuality only 2100 of the 10500 images are exact duplicates.
#    However, none of the sub folders are entirely full of duplicates.
#    Therefore, the 'images' folder is marked as problem=5
...
DATASET_URL_1 = "https://www.kaggle.com/datasets/apollo2506/landuse-scene-classification/download?datasetVersionNumber=3"
DATASET_FILE_1 = "archive.zip"
DATASET_SUM_1 = "7887af8291dbe0b21e1e31fb797cfcac"
...
def parse_dataset(
    ...
    # class/label list (via label_map.json)
    with open(dataset_root_path + "original_files/label_map.json") as label_file:
        class_list = json.load(label_file)
    ...
    for image_path in image_path_list:
        ...
        # many exact duplicates, many not-exact duplicates
        if "/images/" in image_path:
            problem_value = 5

        if "images_train_test_val/train" in image_path:
            set_value = 0
        elif "images_train_test_val/validation" in image_path:
            set_value = 1
        elif "images_train_test_val/test" in image_path:
            set_value = 2
