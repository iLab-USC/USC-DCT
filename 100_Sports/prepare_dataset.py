...
#
# 1) some images to predict given with no label
...
DATASET_URL_1 = "https://www.kaggle.com/datasets/gpiosenka/sports-classification"
DATASET_FILE_1 = "archive.zip"
DATASET_SUM_1 = "f34a0d36da174b64922ba1cb7ca4b0d9"
...
def parse_dataset(
    ...
    # get class names from class_dict.csv
    with open(dataset_root_path + "original_files/" + "class_dict.csv", "r") as f:
        rows = f.readlines()
        class_idxs = [row.split(",")[0] for row in rows[1:]]
        class_names = [row.split(",")[1] for row in rows[1:]]

    # get class names from directory structure
    test_directories = os.listdir(dataset_root_path + "original_files/" + "test")
    train_directories = os.listdir(dataset_root_path + "original_files/" + "train")
    valid_directories = os.listdir(dataset_root_path + "original_files/" + "valid")

    # check if class folders match for each split
    assert set(train_directories) == set(class_names)
    assert set(test_directories) == set(class_names)
    assert set(valid_directories) == set(class_names)

    # update class_dict with class indices
    for class_name, class_idx in zip(class_names, class_idxs):
        class_dict.update({class_name: int(class_idx)})
    ...
    for image_path in image_path_list:
        ...
        # set problem and set for test images with no label
        if "images to predict" in image_path:
            problem_value = 4
            set_value = 2

        # according to folder, set images to test or validation
        if image_path.split("/")[1] == "test":
            set_value = 2
        elif image_path.split("/")[1] == "valid":
            set_value = 1
