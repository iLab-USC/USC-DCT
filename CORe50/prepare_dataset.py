...
# 1) no train/test/val/split
# 2) the class_name come from core_50_class_names.txt
# 3) infer class label for images from the directory structure
# 4) 2 class names have typos, but are left as-is ("glass" and "scissor")
...
DATASET_URL_1 = "http://bias.csr.unibo.it/maltoni/download/core50/core50_350x350.zip"
DATASET_FILE_1 = "core50_350x350.zip"
DATASET_SUM_1 = "e304258739d6cd4b47e19adfa08e7571"

DATASET_URL_2 = "https://vlomonaco.github.io/core50/data/core50_class_names.txt"
DATASET_FILE_2 = "core50_class_names.txt"
DATASET_SUM_2 = "bd46849427c99ef1e51f440c77afd6ce"
#
# 1. imports
#
...
def parse_dataset(
    ...
    run_cmd_get_output(
        [
            "cp",
            dataset_root_path + "archives/core50_class_names.txt",
            dataset_root_path + "original_files/",
        ]
    )
    with open(dataset_root_path + "original_files/core50_class_names.txt") as ifile:
        class_list = ifile.readlines()
        class_list = [line.strip("\n") for line in class_list]

    class_instance_dict = dict()
    for k, class_name in enumerate(sorted(class_list, key=str.casefold)):
        class_instance_dict.update({class_name: k})

    label_class_dict = dict()
    for k, v in class_instance_dict.items():
        label_class_dict.update({v: k})

    # strip out the instance id for each class
    # and remove underscore (per website)
    class_list = [
        name[:-1].replace("_", " ")
        for name in class_list
        if name[:-1] not in class_list
    ]
    ...
    for image_path in image_path_list:
        ...
        if problem_value != 0:
            ...
            # account for starting from 0
            class_from_path = int(class_from_path[1:]) - 1
            class_from_path = class_list[class_from_path]
