...
# 1) under the original/chest_xray/ directory, original/chest_xray/_MACOSX,
# and original/chest_xray/chest_xray each contains a duplicate of the train, val, test
...
DATASET_SUM_1 = "930763e3580e76de9c2c849ec933b5d6"
...
def parse_dataset(
    ...
    class_list = os.listdir(
        dataset_root_path + "original_files/" + "chest_xray/" + "train/"
    )
    ...
    for image_path in image_path_list:
        ...
        if "chest_xray/chest_xray/" in image_path:
            problem_value = 3
        if "__MACOSX" in image_path:
            problem_value = 3

        # set split to test if image is in the test set folder
        if "chest_xray/val" in image_path:
            set_value = 1
        if "chest_xray/test" in image_path:
            set_value = 2
