...
DATASET_URL_1 = "https://prod-dcd-datasets-cache-zipfiles.s3.eu-west-1.amazonaws.com/4drtyfjtfy-1.zip"
DATASET_FILE_1 = "4drtyfjtfy-1.zip"
DATASET_SUM_1 = "76ae8c0ac1089993639008e376414535"
...
def extract_dataset(dataset_root_path: str):
    ...
    # this archive has another archive in it that needs to be unzippped :/
    original_file_path = dataset_root_path + "original_files/" + "dataset2.zip"
    run_cmd_get_output(["unzip", "-qq", original_file_path])
...
def parse_dataset(
    ...
    # hard code class names from the files
    ...
    for image_path in image_path_list:
        ...
        # make sure every image has the class name embedded in it
        # record class name if it does
        matched_to_class = False
        class_name_img = None
        for c in class_list:
            if c in image_path:
                matched_to_class = True
                class_name_img = c
        assert matched_to_class == True
        ...
        if problem_value != 0:
            ...
            assert class_name_img in class_dict
            class_int = class_dict[class_name_img]
