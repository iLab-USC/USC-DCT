...
# has train.val.test split
# label start from 1 (from classes.txt file)
...
DATASET_URL_1 = "https://www.kaggle.com/datasets/rtlmhjbn/ip02-dataset/download?datasetVersionNumber=1"
DATASET_FILE_1 = "archive.zip"
DATASET_SUM_1 = "cd3a1fa40c8524485bd430850daed9b2"
...
def parse_dataset(
    ...
    # class/label list (via classes.txt)
    with open(dataset_root_path + "original_files/classes.txt") as label_file:
        label_list = label_file.readlines()
        for line in label_list:
            line = line.strip()
            line_list = line.split(" ")
            label_num = int(line_list[0])
            if line_list[1] == "":
                label_name = " ".join(line_list[2:])
            else:
                label_name = " ".join(line_list[1:])
    ...
    for image_path in image_path_list:
        ...
        if "classification/train" in image_path:
            set_value = 0
        elif "classification/val" in image_path:
            set_value = 1
        elif "classification/test" in image_path:
            set_value = 2
        ...
        if problem_value != 0:
            ...
            class_int = int(class_from_path) + 1
