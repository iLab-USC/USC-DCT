...
# 1) class_name inferred from the image_path (e.g. dense_residential_154.jpg)
# 2) no train/test/val split
...
DATASET_URL_1 = "https://1drv.ms/u/s!AmgKYzARBl5ca3HNaHIlzp_IXjs"
DATASET_FILE_1 = "NWPU-RESISC45.rar"
DATASET_SUM_1 = "d824acb73957502b00efd559fc6cfbbb"
...
def parse_dataset(
    ...
    class_list = set(
        [
            "_".join(re.split(r"_", img_path.split("/")[-1])[:-1])
            for img_path in image_path_list
        ]
    )

    # update class_dict with class indices
    for k, class_name in enumerate(sorted(list(class_list))):
    ...
    for image_path in image_path_list:
        ...
        if problem_value != 0:
            ...
            class_from_path = re.split(r"_", class_from_path)
            class_from_path = "_".join(class_from_path[:-1])
