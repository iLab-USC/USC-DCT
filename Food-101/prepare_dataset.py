...
#
# 1) no initial issues found
# 2) used given train/test splits
# 3) there are two text files that are related to the classes: 'classes.txt' and 'labels.txt'
#    however, because classes.txt seems to be explicitly the class names, it will be used
#    (labels.txt could be used for a guide for more "friendly" names later)
...
DATASET_URL_1 = "http://data.vision.ee.ethz.ch/cvl/food-101.tar.gz"
DATASET_FILE_1 = "food-101.tar.gz"
DATASET_SUM_1 = "85eeb15f3717b99a5da872d97d918f87"
...
def parse_dataset(
    ...
    class_list_file = (
        dataset_root_path + "original_files/" + "food-101/meta/classes.txt"
    )
    with open(class_list_file, "r") as f:
        rows = f.readlines()
        class_list = [row.strip() for row in rows]

    # run sanity check that class names match
    assert set(directories) == set(class_list)

    # update class_dict with class indices
    for k, class_name in enumerate(sorted(class_list)):
        class_dict.update({class_name: k})

    # read test split files
    relative_imgs_path = "original_files/food-101/images/"
    test_file = dataset_root_path + "original_files/" + "food-101/meta/test.txt"
    with open(test_file, "r") as f:
        rows = f.readlines()
        test_set = set([relative_imgs_path + row.strip() + ".jpg" for row in rows])
    ...
    for image_path in image_path_list:
        ...
        # set split to test if image is in test.txt
        if image_path in test_set:
            set_value = 2
