...
from dataset_utils import VALID_IMAGE_EXTENSIONS, DatasetImage
...
IMAGES_ROOT_PATH = "/lab/tmpig15b/u/DCT_files/"
VECTORS_ROOT_PATH = "/lab/tmpig15b/u/DCT_vectors/"
DATABASE_PATH = "/lab/tmpig15b/u/DCT_files/0_collection/all_images.sqlite"
...
class CollectionDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> tuple[Tensor, int]:
        ...
        if self.input_type == "features":
            ...
            if self.pipeline:
                image_tensor = self.pipeline(image_tensor)
    ...
    @staticmethod
    def execute_database_query(query: str) -> list[Any]:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("need to at least specify which test to run:")
        print("    -every          for verify_everything")
        print("    -original       for verify_all_original_images_are_in_database")
        sys.exit()

    if len(sys.argv) > 2:
        # if an argument is provided, assume it is the root_path
        root_path = str(sys.argv[2])
    else:
        # assume the root_path is the current directory
        root_path = os.getcwd()

    check_to_run = str(sys.argv[1])
    if check_to_run == "-every":
        CollectionDataset.verify_everything(root_path)
    elif check_to_run == "-original":
        CollectionDataset.verify_all_original_images_are_in_database(root_path)
    else:
        print("invalid option:", check_to_run)
