...
#
# 1) dataset entirely replicated as a class folder
#    any file under simpsons_dataset/simpsons_dataset should be ignored as redundant
# 2) annotation.txt does not have all classes that appear as folders, ignored while producing list
# 3) test set seems to have fewer classes than in the dataset as well
...
DATASET_SUM_1 = "c97abe815f7639d7e001339bb590ec3b"
...
def parse_dataset(
    ...
    class_list = os.listdir(dataset_root_path + "original_files/" + "simpsons_dataset")

    # remove dataset duplication from class list
    class_list.remove("simpsons_dataset")
    ...
    for image_path in image_path_list:
        ...
        if "simpsons_dataset/simpsons_dataset" in image_path:
            problem_value = 3

        # set split to test if image is in the test set folder
        if "kaggle_simpson_testset" in image_path:
            set_value = 2
        ...
        if problem_value != 0:
            ...
            if set_value == 2:
                ...
                class_from_path = "_".join(image_name.split("_")[:-1])
            else:
