...
#
# 1) the labels are included in the train.csv file
# 2) test set has no labels
# 3) the extracted files has no class name information. So hard-coded from the original website
#    https://www.kaggle.com/competitions/aptos2019-blindness-detection/
# 4) class_ids came from the train.csv file
...
DATASET_URL_1 = "https://www.kaggle.com/competitions/aptos2019-blindness-detection/data?select=train.csv"
DATASET_FILE_1 = "aptos2019-blindness-detection.zip"
DATASET_SUM_1 = "fff5440648ab9ee707bc56c97c40dc2f"
...
def parse_dataset(
    ...
    # get the train csv and convert it into dict with k:v = img:label
    train_df = pd.read_csv(dataset_root_path + "original_files/" + "train.csv")
    train_dict = train_df.set_index("id_code").to_dict(orient="index")
    train_dict = {k: v["diagnosis"] for k, v in train_dict.items()}

    # hard-code the class dict
    class_dict.update({"No DR": 0})
    class_dict.update({"Mild": 1})
    class_dict.update({"Moderate": 2})
    class_dict.update({"Severe": 3})
    class_dict.update({"Proliferative DR": 4})
    ...
    for image_path in image_path_list:
        ...
        # set the set_value (train or val or test) and the problem value for test images
        if "test_images" in image_path:
            set_value = 2
            problem_value = 4
        ...
        if problem_value != 0:
            ...
            # strip the .png from the parse
            img_from_path = img_from_path[: img_from_path.find(".")]
            if img_from_path in train_dict:
                class_id = train_dict[img_from_path]
            else:
                class_id = -1
                problem_value = 4
