...
# 1. constants
#
DATASET_URL_1 = "https://www.robots.ox.ac.uk/~vgg/data/flowers/102/102flowers.tgz"
DATASET_FILE_1 = "102flowers.tgz"
DATASET_SUM_1 = "52808999861908f626f3c1f4e79d11fa"

DATASET_URL_2 = "https://www.robots.ox.ac.uk/~vgg/data/flowers/102/imagelabels.mat"
DATASET_FILE_2 = "imagelabels.mat"
DATASET_SUM_2 = "e0620be6f572b9609742df49c70aed4d"

DATASET_URL_3 = "https://www.robots.ox.ac.uk/~vgg/data/flowers/102/setid.mat"
DATASET_FILE_3 = "setid.mat"
DATASET_SUM_3 = "a5357ecc9cb78c4bef273ce3793fc85c"

DATASET_URL_4 = (
    "https://github.com/MAbdelhamid2001/Image-Classifier/blob/main/label_map.json"
)
DATASET_FILE_4 = "label_map.json"
DATASET_SUM_4 = "dc987f9e865ea96f02e6358d86e64f40"
...
def verify_md5sums(dataset_root_path: str) -> bool:
    ...
    md5sum_result4 = run_cmd_get_output(["md5sum", DATASET_FILE_4]).strip().split()[0]
    ...
    return (
        ...
        and md5sum_result4 == DATASET_SUM_4
    )
...
def extract_dataset(dataset_root_path: str):
    ...
    run_cmd_get_output(
        ["cp", "../archives/setid.mat", "."]
    )  # copy setid to split train/test/val
    run_cmd_get_output(
        ["cp", "../archives/imagelabels.mat", "."]
    )  # copy setid to split train/test/val
    run_cmd_get_output(
        ["cp", "../archives/label_map.json", "."]
    )  # copy setid to split train/test/val
...
def parse_dataset(
    ...
    imagelabels_mat = scipy.io.loadmat(
        dataset_root_path + "original_files/imagelabels.mat"
    )
    imagelabels = list(imagelabels_mat["labels"][0])  # ndarray:(1, 8189)

    # load the train val split
    setid_mat = scipy.io.loadmat(dataset_root_path + "original_files/setid.mat")
    train_id = list(setid_mat["trnid"][0])  # ndarray:(1, 1020)
    val_id = list(setid_mat["valid"][0])  # ndarray:(1, 1020)
    test_id = list(setid_mat["tstid"][0])  # ndarray:(1, 6149)

    for i, img_name in enumerate(image_path_list):  # for each image path
        # find the image index
        img_index = int(img_name.split("_")[-1].split(".")[0])
        # find imagelabel based on index
        img_label = int(
            imagelabels[img_index - 1]
        )  # image label start from 0, index start from 1

        # find split dataset infobased on index
        if img_index in train_id:
            img_set_id = 0
        elif img_index in val_id:
            img_set_id = 1
        else:
            img_set_id = 2
    ...
    # load label_map
    f = open(dataset_root_path + "original_files/label_map.json")
    label_map = json.load(f)

    # update class_dict with class indices
    for class_name in sorted(label_map.items(), key=lambda item: int(item[0])):
        class_dict.update({class_name[-1]: int(class_name[0])})
