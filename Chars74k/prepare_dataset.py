...
# 2 no provided label_name to label index mapping, so create one use alphabeta sequence, using the folder name as class name
# 3 masks are ignored as problem 6
# 4 images in BadImag folder look difficult but are added to dataset
...
DATASET_URL_1 = "http://www.ee.surrey.ac.uk/CVSSP/demos/chars74k/EnglishImg.tgz"
DATASET_FILE_1 = "EnglishImg.tgz"
DATASET_SUM_1 = "85d157e0c58f998e1cda8def62bcda0d"
...
def parse_dataset(
    ...
    class_list = os.listdir(
        dataset_root_path + "original_files/" + "English/Img/BadImag/Bmp"
    )
    ...
    for i, img_name in enumerate(image_path_list):  # for each image path
        ...
        if not is_image_valid(dataset_root_path + img_name):
            ...
            problem_value = 1
        elif img_name.split("/")[4] == "Msk":
            problem_value = 6  # they are mask
        else:
            problem_value = 0
        ...
        if problem_value != 0:
            ...
            assert img_name.split("/")[5] in class_dict
            img_label = class_dict[img_name.split("/")[5]]
