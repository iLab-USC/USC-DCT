...
DATASET_SUM_1 = "fd11132ef28fd1f041be9cc8ccdc6634"

# map is taken from website
...
def parse_dataset(
    ...
    class_dict = {val: key for key, val in CLASS_MAP.items()}
    ...
    train_df = pd.read_csv(f"{dataset_root_path}/original_files/train_set_labels.csv")
    test_df = pd.read_csv(f"{dataset_root_path}/original_files/test_set_labels.csv")

    train_dict = {
        train_df["Label"][i]: int(train_df["Species"][i]) for i in range(len(train_df))
    }
    test_dict = {
        test_df["Label"][i]: int(test_df["Species"][i]) for i in range(len(test_df))
    }
    ...
    for image_path in image_path_list:
        ...
        if file_name not in test_dict and file_name not in train_dict:
            problem_value = 4

        if "deepweeds_images_256" in image_path:
            problem_value = 3
        ...
        if problem_value != 0:
            ...
            if file_name in test_dict:
                set_value = 2
                class_id = test_dict[file_name]
            else:
                class_id = train_dict[file_name]
