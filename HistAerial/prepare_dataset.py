...
# 1) Although train/val/test are given, they are sampled from original dataset. Hence for full dataset, all are considered as train
# 2) Per paper, "no overlapping between the patches of a same size", along with
#    other details, implies that there is overlap between the patches of different sizes.
#    Therefore, since 25x25 and 50x50 images are taken from the 100x100, they will be marked
#    as problem=6 (different task)
...
DATASET_URL_1 = "http://eidolon.univ-lyon2.fr/~remi1/HistAerialDataset/dataset/HistAerialDataset.zip"
DATASET_FILE_1 = "HistAerialDataset.zip"
DATASET_SUM_1 = "cff202e58d8905cf108ae0cc9cee9feb"
...
def parse_dataset(
    ...
    class_list = os.listdir(
        dataset_root_path + "original_files/" + "100x100_overlap_0percent"
    )
    for l in list(class_list):
        if len(l.split(".")) == 2:
            class_list.remove(l)
    ...
    for image_path in image_path_list:
        ...
        # flag redundant image 25x25 and 50x50
        if ("25x25" in image_path) or ("50x50" in image_path):
            problem_value = 6
