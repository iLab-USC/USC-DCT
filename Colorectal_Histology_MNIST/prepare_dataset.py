...
# 1) the extracted content contains 2 same copies: /kather_texture_2016_image_tiles_5000
# 2) the extracted content contains 2 same copies: /kather_texture_2016_image_tiles_10
# that does not have labels and the image is larger
# 3) the .csv file contain pixel level information (possibly the intensity of the pixel)
# not used for determing the labels
# (4) the labels are inferred from the directory structure
...
DATASET_URL_1 = "https://www.kaggle.com/datasets/kmader/colorectal-histology-mnist/download?datasetVersionNumber=2"
DATASET_FILE_1 = "archive.zip"
DATASET_SUM_1 = "e03501016bd54719567dfb954fe982fe"
...
def parse_dataset(
    ...
    for k, class_name in enumerate(sorted(class_list)):
        class_dict.update({class_name[3:]: k})
    ...
    for image_path in image_path_list:
        ...
        if "Kather_texture_2016_larger_images_10" in image_path:
            problem_value = 4
        elif image_path in existing_paths:
            problem_value = 3
        else:
            existing_paths.add(image_path)

        # mark the duplicate images as dupes
        if (
            "Kather_texture_2016_image_tiles_5000/Kather_texture_2016_image_tiles_5000"
            in image_path
        ):
        ...
        if problem_value != 0:
            ...
            class_from_path = image_path.split("/")[-2][3:]
