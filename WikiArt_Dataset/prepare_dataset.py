...
#
# 1) one file had a bad CRC:
#    wikiart/Baroque/rembrandt_woman-standing-with-raised-hands.jpg  CRC 3d470143  (should be 90cccc50)
#    it should be noted that two different URLs are provided for the dataset,
#    however, they both produce the same md5sum (so are the same)
# 2) the dataset contains two very large images that pillow raises warnings about
#    wikiart/Color_Field_Painting/barnett-newman_uriel-1955.jpg
#    wikiart/Color_Field_Painting/barnett-newman_vir-heroicus-sublimis-1950.jpg
# 3) the class names come from the structure
...
DATASET_FILE_1 = "wikiart.zip"
DATASET_SUM_1 = "c6b43cbcd474b875a5626ffde3b627e1"
...
def extract_dataset(dataset_root_path: str):
    ...
    # don't check the output, as there is one file that unzip says has a bad crc
    run_cmd_get_output(["unzip", "-qq", original_location], check=False)
